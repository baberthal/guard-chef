...
RSpec.describe Guard::Chef::Inspectors::BaseInspector do
  let(:options) { { custom: "value", spec_paths: %w(myspec) } }
  let(:inspector) { Guard::Chef::Inspectors::BaseInspector.new(options) }
  ...
  describe "#paths" do
    ...
    context "specific inspector" do
      class FooInspector < Guard::Chef::Inspectors::BaseInspector
        def paths(paths)
          _clean(paths)
        end
      end
        ...
        context "chdir option present" do
          ...
          it "returns matching paths" do
            allow(Dir).to receive(:[])
              .with("moduleA/spec/**{,/*/**}/*[_.]spec.rb").and_return(paths)

            allow(Dir).to receive(:[])
              .with("moduleA/spec/**{,/*/**}/*.feature").and_return([])
      ...
      context "with spec files" do
        ...
        context "chdir option present" do
          ...
          it "returns matching paths" do
            ...
            allow(File).to receive(:directory?)
              .with("spec/models/a_foo_spec.rb").and_return(false)
