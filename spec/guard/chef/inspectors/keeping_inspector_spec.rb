...
klass = Guard::Chef::Inspectors::KeepingInspector
...
RSpec.describe klass do
  ...
  let(:other_failed_locations) do
    [
      './spec/guard/chef/runner_spec.rb:12',
      './spec/guard/chef/runner_spec.rb:100',
      './spec/guard/chef/inspectors/simple_inspector_spec.rb:12'
    ]
  end
  ...
  it "remembers failed paths and returns them along with new paths" do
    ...
    expect(inspector.paths(other_paths)).to match_array([
      "spec/guard/chef/deprecator_spec.rb",
      "spec/guard/chef/runner_spec.rb",
      "spec/guard/chef/inspectors/simple_inspector_spec.rb"
    ])
    ...
    expect(
      inspector.paths(
        %w(spec/guard/chef/inspectors/base_inspector_spec.rb)
      )
    ).to match_array([
      "spec/guard/chef/runner_spec.rb",
      "spec/guard/chef/inspectors/simple_inspector_spec.rb",
      "spec/guard/chef/inspectors/base_inspector_spec.rb"
    ])
    inspector.failed(other_failed_locations)

    expect(inspector.paths(%w(spec/guard/chef/runner_spec.rb)))
      .to match_array([
        "spec/guard/chef/runner_spec.rb",
        "spec/guard/chef/inspectors/simple_inspector_spec.rb"
      ])
...
#    expect(inspector.paths(other_paths)).to match_array(
#      other_paths +
#      %w[./spec/guard/chef/deprecator_spec.rb:55]
#    )
#    inspector.failed(other_failed_locations)
#
#    # Now it returns other failed locations
#    expect(inspector.paths(%w[spec/guard/chef/deprecator_spec.rb])).to
#    match_array(
#      other_failed_locations +
#      %w[spec/guard/chef/deprecator_spec.rb]
#    )
#    inspector.failed(other_failed_locations)
#
