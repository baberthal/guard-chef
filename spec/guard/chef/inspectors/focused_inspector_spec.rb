...
klass = Guard::Chef::Inspectors::FocusedInspector
...
RSpec.describe klass do
  ...
  let(:other_failed_locations) do
    %w(./spec/guard/chef/deprecator_spec.rb:446)
  end
  ...
  it "remembers failed paths and returns them until they all pass" do
    ...
    allow(File).to receive(:directory?)
      .with("spec/guard/chef/runner_spec.rb").and_return(false)

    allow(File).to receive(:directory?)
      .with("spec/guard/chef/deprecator_spec.rb").and_return(false)
